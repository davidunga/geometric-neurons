...
def pair_to_index(n: int, item1: int, item2: int) -> int:
    return - (item1 * (item1 + 3 - 2 * n) + 2) // 2 + item2


def index_to_pair(n: int, index: int) -> tuple[int, int]:
    i = int(-.5 * np.sqrt((1 - 2 * n) ** 2 - 8 * index) + n - .5)
    j = index - pair_to_index(n, i, 0)
    return i, j


class PairToIndex:
    """ Fast convert pair to index
        Example:
            p2i = PairToIndex(n)
            indexes = [p2i(i, j) for i, j in combinations(range(n), 2)]
    """

    def __init__(self, n: int):
        # indexes for (0, 1), (1, 2), .. (n - 1, n)
        self.index_0 = np.r_[0, np.cumsum(range(n - 1, 1, -1))] - np.arange(n - 1) - 1

    def __call__(self, i: int, j: int) -> int:
        return self.index_0[i] + j
...
if __name__ == "__main__":
    ...
    n = 10
    p = PairToIndex(n)
    for index, (i, j) in enumerate(iter_pairs(n)):
        print(index, p(i, j), (i, j))

    # tic = time()
    # for _ in range(1000):
    #     x = list([iter_pairs2(50)])
    # print(time() - tic)
    # tic = time()
    # for _ in range(1000):
    #     x = list([iter_pairs(50)])
    # print(time() - tic)
    ...
    #
    #     for index, (i, j) in enumerate(iter_pairs(n)):
    #         assert pair_to_index(n, i, j) == index
    #         assert index_to_pair(n, index) == (i, j)
    #         print(pair_to_index(n, i, j), index, (i, j), index_to_pair(n, index))
